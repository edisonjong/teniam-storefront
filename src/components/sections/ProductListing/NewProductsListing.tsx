...
import {
  ...
  NewProductsListingProductsView,
...
export const NewProductsListing = ({
  category_id,
  collection_id,
  seller_handle,
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      try {
        ...
        updateFacets(result.facets) // Update the global facets context
        setCount(result.nbHits)
        setPages(result.nbPages)
      } catch (error) {
        setProducts([])
        setFacets({})
        updateFacets({}) // Clear the global facets context on error
        setCount(0)
        setPages(0)
      } finally {
        setIsLoading(false)
    ...
    fetchProducts()
  }, [locale, filters, query, page, currency_code, updateFacets])
  ...
  return (
    <div className="min-h-[70vh]">
      ...
      <div className="md:flex gap-4">
        {/* 
          Commented out the Algolia filter sidebar to rely fully on the drawer for filtering.
          The drawer provides a cleaner, more mobile-friendly interface.
        */}
        ...
        <div className="w-full flex flex-col">
          ...
          {!isLoading && products.length > 0 && (
            <NewProductsListingProductsView products={products} />
          )}
